...
{
    (*pp_poly) = (Polynomial*)trp_gc_malloc( sizeof(Polynomial) );
    (*pp_poly)->coef = (mpz_t*)trp_gc_malloc( (deg + 1) * sizeof( mpz_t ) );
    (*pp_poly)->deg = deg;
    unsigned int i;
    for ( i = 0; i <= deg; i++ )
...
{
    (*pp_poly_cloned) = (Polynomial*)trp_gc_malloc( sizeof(Polynomial) );
    (*pp_poly_cloned)->coef = (mpz_t*)trp_gc_malloc( (p_poly->deg + 1) * sizeof(mpz_t) );
...
void compact_polynomial( Polynomial* p_poly )
{
    ...
    if (i < p_poly->deg) {
        ...
        p_poly->coef = (mpz_t*)trp_gc_realloc(p_poly->coef, sizeof(mpz_t) * (i + 1));
        p_poly->deg = i;
    }
}
                          ...
                          mpz_t* p_coef )
{
    ...
    p_poly->coef = (mpz_t*)trp_gc_realloc( p_poly->coef, sizeof(mpz_t) * (order + 1) );
    unsigned int i;
