...
uns8b trp_pix_load_jpg_memory( uns8b *idata, uns32b isize, uns32b *w, uns32b *h, uns8b **data )
{
    ...
    if ( isize < 10 )
        return 1;
    if ( ( idata[ 0 ] != 0xff ) || ( idata[ 1 ] != 0xd8 ) || ( idata[ 2 ] != 0xff ) )
        return 1;
    if ( ( ep = epeg_memory_open( idata, isize ) ) == NULL )
        return 1;
    epeg_size_get( ep, &ws, &hs );
    epeg_decode_size_set( ep, ws, hs );
    epeg_decode_colorspace_set( ep, EPEG_RGBA8 );
    *data = epeg_pixels_get( ep, 0, 0, ws, hs );
    epeg_close( ep );
    if ( *data == NULL )
        return 1;
    *w = (uns32b)ws;
...
uns8b trp_pix_save_jpg( trp_obj_t *pix, trp_obj_t *path, trp_obj_t *quality )
{
    ...
    if ( fp == NULL )
        return 1;
    color = ( trp_pix_colors_type( (trp_pix_t *)pix, 0 ) & 0xfffe == 0xfffe ) ? 0 : 1;
    w = ((trp_pix_t *)pix)->w;
    h = ((trp_pix_t *)pix)->h;
