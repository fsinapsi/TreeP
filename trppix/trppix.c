...
static trp_obj_t *trp_pix_load_memory_low( trp_obj_t *raw, uns32b w, uns32b h, trp_obj_t *cnt );
...
objfun_t _trp_pix_rotate_lept = NULL;
objfun_t _trp_pix_rotate_cv = NULL;
...
trp_obj_t *trp_pix_info( trp_obj_t *path )
{
    ...
    if ( trp_pix_info_png( cpath, &w, &h ) )
        if ( trp_pix_info_jpg( cpath, &w, &h ) )
            if ( trp_pix_info_pnm( cpath, &w, &h ) )
                if ( trp_pix_info_gif( cpath, &w, &h ) ) {
                } else
    ...
    if ( sottotipo )
        return trp_cons( trp_cons( trp_sig64( (sig64b)w ), trp_sig64( (sig64b)h ) ),
                         trp_cord( _trp_pix_format[ sottotipo - 1 ] ) );
    return UNDEF;
}
...
static trp_obj_t *trp_pix_load_memory_low( trp_obj_t *raw, uns32b w, uns32b h, trp_obj_t *cnt )
{
    ...
    if ( raw->tipo != TRP_RAW )
        return UNDEF;
    if ( cnt ) {
        if (  trp_cast_uns32b( cnt, &c ) )
            return UNDEF;
        if ( c > ((trp_raw_t *)raw)->len )
            c = ((trp_raw_t *)raw)->len;
    } else {
        c = ((trp_raw_t *)raw)->len;
    }
...
//         ( s[ 3 ] == 0xfe )
        ...
        if ( ep = epeg_memory_open( s, c ) ) {
            ...
            if ( p = epeg_pixels_get( ep, 0, 0, ws, hs ) ) {
                w = (uns32b)ws;
                h = (uns32b)hs;
            }
            epeg_close( ep );
    ...
    if ( p == NULL )
        if ( trp_pix_load_png_memory( (trp_raw_t *)raw, &w, &h, (uns8b **)( &p ) ) )
            p = NULL;
    if ( p == NULL )
        if ( w * h * 3 == c ) {
            uns8b *d;
    ...
    if ( p == NULL )
        return UNDEF;
    return trp_pix_create_image_from_data( 0, w, h, (uns8b *)p );
}

trp_obj_t *trp_pix_load_memory( trp_obj_t *raw, trp_obj_t *cnt )
{
    return trp_pix_load_memory_low( raw, 0, 0, cnt );
}
...
trp_obj_t *trp_pix_load_memory_ext( trp_obj_t *raw, trp_obj_t *w, trp_obj_t *h, trp_obj_t *cnt )
{
    ...
    if ( trp_cast_uns32b( w, &ww ) ||
        ...
        return UNDEF;
    return trp_pix_load_memory_low( raw, ww, hh, cnt );
}
...
//         ( s[ 3 ] == 0xfe )
    ...
    if ( p == NULL )
        if ( trp_pix_load_png_memory( (trp_raw_t *)raw, &www, &hhh, (uns8b **)( &p ) ) )
            p = NULL;
        else {
            ws = www;
            hs = hhh;
...
trp_obj_t *trp_pix_image_format( trp_obj_t *obj )
{
    ...
    if ( obj->tipo != TRP_PIX )
        return UNDEF;
    if ( ((trp_pix_t *)obj)->sottotipo == 0 )
        return UNDEF;
    return trp_cord( _trp_pix_format[ ((trp_pix_t *)obj)->sottotipo - 1 ] );
}
...
trp_obj_t *trp_pix_get_luminance( trp_obj_t *pix )
{
    ...
    if ( ( map = ((trp_pix_t *)pix)->map.c ) == NULL )
        return UNDEF;
    for ( tot_r = tot_g = tot_b = 0, n = ((trp_pix_t *)pix)->w * ((trp_pix_t *)pix)->h, i = 0 ; i < n ; i++, map++ ) {
        tot_r += map->red;
        tot_g += map->green;
        tot_b += map->blue;
    }
    return trp_math_ratio( trp_sig64( tot_r * 299 + tot_g * 587 + tot_b * 114 ),
                           trp_sig64( n ),
                           trp_sig64( 1000 ),
                           NULL );
}
...
trp_obj_t *trp_pix_get_contrast( trp_obj_t *pix )
{
    ...
    if ( ( map = ((trp_pix_t *)pix)->map.c ) == NULL )
        return UNDEF;
    for ( tot1 = 0, tot2 = 0, n = ((trp_pix_t *)pix)->w * ((trp_pix_t *)pix)->h, i = 0 ; i < n ; i++, map++ ) {
        c = ( map->red * 299 +
              map->green * 587 +
              map->blue * 114 + 500 ) / 1000;
        tot1 += c;
        tot2 += c * c;
    }
    tn = trp_sig64( n );
