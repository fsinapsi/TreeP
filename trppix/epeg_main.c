...
epeg_file_open(const char *file)
{
   ...
   im->in.f = trp_fopen(im->in.file, "rb");
   if (!im->in.f)
     {
	epeg_close(im);
	return NULL;
     }
   trp_fstat(fileno(im->in.f), &(im->stat_info));
   im->out.quality = 75;
...
epeg_pixels_get(Epeg_Image *im, int x, int y,  int w, int h)
{
   ...
   if (_epeg_scale(im) != 0) return NULL; /* Frank Sinapsi 2009-11-25 */
...
_epeg_decode(Epeg_Image *im)
{
   ...
   /* im->in.jinfo.dct_method          = JDCT_IFAST; */ /* commented by Frank Sinapsi 16-06-11 */
...
_epeg_encode(Epeg_Image *im)
{
   ...
   if (im->out.file)
     {
	im->out.f = trp_fopen(im->out.file, "wb");
	if (!im->out.f)
   	  {
	     im->error = 1;
	     return 1;
