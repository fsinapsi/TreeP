...
static uns8b trp_pix_load_png_low( uns8b *cpath, uns8b *idata, uns32b isize, uns32b *w, uns32b *h, uns8b **data );

typedef struct {
    uns8b *idata;
    uns32b isize;
    uns32b read;
} trp_pix_png_memory_t;

static void trp_pix_load_png_memory_cb( png_structp png_ptr, png_bytep outBytes, png_size_t byteCountToRead )
{
    trp_pix_png_memory_t *r = (trp_pix_png_memory_t *)png_get_io_ptr( png_ptr );

    if ( r->read + byteCountToRead > r->isize )
        byteCountToRead = r->isize - r->read;
    if ( byteCountToRead ) {
        memcpy( outBytes, r->idata + r->read, byteCountToRead );
...
static uns8b trp_pix_load_png_low( uns8b *cpath, uns8b *idata, uns32b isize, uns32b *w, uns32b *h, uns8b **data )
{
    ...
    if ( cpath ) {
        ...
        if ( isize < PNG_BYTES_TO_CHECK )
            return 1;
        if ( png_sig_cmp( idata, (png_size_t)0, PNG_BYTES_TO_CHECK ) )
            return 1;
        r.idata = idata;
        r.isize = isize;
...
{
    return trp_pix_load_png_low( cpath, NULL, 0, w, h, data );
}

uns8b trp_pix_load_png_memory( uns8b *idata, uns32b isize, uns32b *w, uns32b *h, uns8b **data )
{
    return trp_pix_load_png_low( NULL, idata, isize, w, h, data );
}
...
uns8b trp_pix_save_png( trp_obj_t *pix, trp_obj_t *path )
{
    ...
    switch ( trp_pix_colors_type( (trp_pix_t *)pix, 0 ) ) {
    case 0xfffe:
        bpl = ( w >> 3 ) + ( ( w & 7 ) ? 1 : 0 );
